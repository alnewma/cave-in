...
func initiate_polygon(polygon) -> void:
	_polygon = polygon
	...
	for i in range(triangle_count):
		...
		_cumulated_triangle_areas[i] = _cumulated_triangle_areas[i - 1] + PolygonRandomPointGenerator.triangle_area(a, b, c)


func get_random_point(_polygonUsed) -> Vector2:
	initiate_polygon(_polygonUsed)
	...
	return PolygonRandomPointGenerator.random_triangle_point(a, b, c)
